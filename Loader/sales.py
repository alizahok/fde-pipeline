...
def load_sales():
    ...
    try:
        ...
        if 'transform_schema' not in schemas or not schemas['transform_schema']:
            raise RuntimeError("Schema 'transform' is not defined in config or does not exist.")

        conn = get_db_connection()

        # Step 1: Clear temp table
        execute_query(conn, f"""
            TRUNCATE TABLE {schemas['transform_schema']}.{entities[ENTITY]['temp_table']}
        """)
        ...
        # Step 4: Calculate missing total_amount
        ...
        # Step 5: Insert into target table with deduplication
